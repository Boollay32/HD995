// =====================  RFCNotes.js  ===================== //
//
// The Notes thread on the RFC detail page. RFC notes are a plain thread:
// the RFC note feed does not carry a client/internal visibility flag, so
// there is no visibility toggle or badge here (this matches the ticket
// Notes tab, which is also a single internal thread).
//
...
// rendering. The card markup + classes mirror the ticket Notes tab so the
// two look consistent (styled by Components/NoteThread.css).
...
const RFCNotes = (() => {
    ...
    const Dom = {
        thread: () => document.getElementById('RFCNotes-Thread'),
    };
    ...
    function init(rfcId) {
        ...
        // Return the load promise so the caller (RFCDetails._loadRFCData) can
        // await the notes alongside the rest of the page data.
        return _getNotes();
    }
    ...
    async function _onSend({ text, files }) {
        ...
        try {
            ...
            const data = await API.post(
                'Note/SaveNote',
                API.authPayload({ objectInfo, attachments, rfc: true })
            );
            ...
            console.error('RFCNotes._onSend:', err);
    ...
    async function _getNotes() {
        try {
            const [data, attMap] = await Promise.all([
                API.post('Note/GetRFCNotes', API.authPayload({ rfcId: State.rfcId })),
                Composer.fetchNoteAttachments(State.rfcId, 1),
            ]);
            ...
            console.error('RFCNotes._getNotes:', err);
    ...
    async function _commitEdit(card, note, newText, working, btns) {
        ...
        try {
            ...
            console.error('RFCNotes._commitEdit:', err);
